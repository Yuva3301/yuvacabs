...
export default function AdminDashboard({ onClose }) {
  ...
  const loadBookings = () => {
    try {
      const data = localStorage.getItem('yuvacabs_bookings');
      if (data) {
        ...
        localStorage.setItem('yuvacabs_bookings', JSON.stringify(mockData));
  ...
  useEffect(() => {
    ...
    window.addEventListener('yuvacabs_booking_submitted', handleRefresh);
    return () => window.removeEventListener('yuvacabs_booking_submitted', handleRefresh);
  }, []);
  ...
  const handleStatusChange = (id, newStatus) => {
    ...
    setBookings(updated);
    localStorage.setItem('yuvacabs_bookings', JSON.stringify(updated));
  };
  ...
  const handlePurge = () => {
    if (window.confirm('WARNING: Are you sure you want to purge all bookings? This cannot be undone.')) {
      localStorage.setItem('yuvacabs_bookings', JSON.stringify([]));
      setBookings([]);
